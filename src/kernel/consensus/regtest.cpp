...
bool CryptoKernel::Consensus::Regtest::mineBlock(const bool isBetter, const std::string &pubKey)
{
	...
	const auto res = blockchain->submitBlock(CryptoKernel::Blockchain::block(blockJson));
	if(!std::get<0>(res)) {
