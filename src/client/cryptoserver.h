...
public:
    CryptoRPCServer(jsonrpc::AbstractServerConnector &conn,
        ...
        this->bindAndAddMethod(jsonrpc::Procedure("mineblock", jsonrpc::PARAMS_BY_NAME,
                              jsonrpc::JSON_BOOLEAN, "isBetter", jsonrpc::JSON_BOOLEAN, 
                              "pubKey", jsonrpc::JSON_STRING, NULL),
                              &CryptoRPCServer::mineblockI);
    ...
    inline virtual void mineblockI(const Json::Value &request, Json::Value &response) {
        response = this->mineblock(request["isBetter"].asBool(), request["pubKey"].asString());
    }
    ...
    virtual bool mineblock(const bool isBetter, const std::string &pubKey) = 0;
...
public:
    ...
    virtual bool mineblock(const bool isBetter, const std::string &pubKey);
